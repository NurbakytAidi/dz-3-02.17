...
// Dz-3 добавить на вторую кнопку модалку


const modal2 = document.querySelector('.modal')
const modalTriggerItem = document.querySelector('.btn_dark')
const closeModalBtnItem = document.querySelector('.modal__close')
...
modalTriggerItem.onclick = () => openModalItem()
closeModalBtnItem.onclick = () => closeModalItem()

modal2.onclick = event => event.target === modal ? closeModalItem() : false
modal2.onkeydown = event => event.code === 'Escape' ? closeModalItem() : false
...
function openModalToScroll() {
    const page = document.documentElement

    if (page.scrollTop + page.clientHeight >= page.scrollHeight) {
        openModal()
        window.removeEventListener("scroll", openModalToScroll)

    }

}

window.addEventListener("scroll", openModalToScroll)
